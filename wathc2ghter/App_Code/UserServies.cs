...
public class UserServies
{
    ...
    public DataTable PopFriends(int UserId)
         {//פעולה מקבלת userid 
        ...
        try
            ...
            parm = Mycommand.Parameters.Add("@UserId", OleDbType.Integer);
                parm.Value = UserId;
                 ...
                 DataTable dt = new DataTable();
                 dt.Columns.AddRange(new DataColumn[] { new DataColumn("FriendId"), new DataColumn("UserName") });
                 while (Reader.Read())
                 {
                     dt.Rows.Add(new object[] { Reader["FriendId"].ToString(), Reader["UserName"].ToString() });
                 }
                 return dt;
             }
